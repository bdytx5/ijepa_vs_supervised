...
def main(args, resume_preempt=False):
    ...
    for data_fraction in [0.1, 0.3, 0.5, 0.75, 1.0]:
        ...
        # # Split the datasets according to data_fraction
        ...
        # train_subset, _ = torch.utils.data.random_split(train_dataset, [train_subset_size, total_train_samples - train_subset_size])
        # val_subset, _ = torch.utils.data.random_split(val_dataset, [val_subset_size, total_val_samples - val_subset_size])

        # train_loader = torch.utils.data.DataLoader(train_subset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_mem)
        # val_loader = torch.utils.data.DataLoader(val_subset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_mem)
        ...
        run = wandb.init(project='supervised_ijepa_tr', entity='byyoung3', name=f'{tag}_fraction_{data_fraction}')
