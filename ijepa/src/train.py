...
#     def validate(val_loader, encoder, predictor, target_encoder, device='cuda:0'):
#         encoder.eval()
#         predictor.eval()
#         target_encoder.eval()
#         val_loss = AverageMeter()

#         def forward_target(imgs, masks_pred):
#             with torch.no_grad():
...
#         def forward_context(imgs, masks_enc, masks_pred):
...
#                     h = forward_target(imgs, masks_pred)
#                     z = forward_context(imgs, masks_enc, masks_pred)
#                     loss = loss_fn(z, h)
...
#     run = wandb.init(project='ijepa_tr', entity='byyoung3')
...
#         # Validation step
#         # val_loss = validate(val_loader, encoder, predictor=target_encoder)
#         val_loss = validate(val_loader, encoder, predictor, target_encoder)
...
#     run.log_model(path=best_model_path, name="ijepa_best_model", aliases=["best"])
#     run.finish()
